...
double my_lagrange(int a, int b, double x, double x_arr[], double* z)
	...
	for (int i = a; i <= b; i++)
		{
		double l = 1;
		for (int j = a; j <= b; j++) if (j != i) l *= (x - x_arr[j]) / (x_arr[i] - x_arr[j]);
		sum += z[i] * l;
		}
	return sum;
	}
...
double New1(double* z, double x)
	...
	int n = 6;
	double x0 = 1, h = 1;
	double d[6] = { d1(z[0],z[1]),
					d2(z[0],z[1],z[2]),
					d3(z[0],z[1],z[2],z[3]),
					d4(z[0],z[1],z[2],z[3],z[4]),
					d5(z[0],z[1],z[2],z[3],z[4],z[5]),
					d6(z[0],z[1],z[2],z[3],z[4],z[5],z[6]) };
...
double New2(double* z, double x)
	...
	int n = 6;
	double x0 = 7, h = 1;
	double d[6] = { d1(z[5],z[6]),
					d2(z[4],z[5],z[6]),
					d3(z[3],z[4],z[5],z[6]),
					d4(z[2],z[3],z[4],z[5],z[6]),
					d5(z[1],z[2],z[3],z[4],z[5],z[6]),
					d6(z[0],z[1],z[2],z[3],z[4],z[5],z[6]) };
	double q = (x - x0) / h;
	sum += q * d[0];
	for (int i = 1; i < n; i++)
...
int main()
	...
	for (int i=0; i<200; i++) temp[i] = new double[14];
	...
	fstream fin;
	fin.open("12_Пермь.csv", ios::in);

	double** y_arr = new double* [str];
	for (int count = 0; count < str; count++) y_arr[count] = new double[stolb];
	...
	ofstream fout;
	fout.open("Lagrange.txt");
	ofstream fout0;
	fout0.open("Lagrange0.txt");

	cout << endl;
	cout << endl;
	cout << "Task number 2 (Lagrange)"	<< endl;
	cout << "+ GHART!!!"	<< endl;
	cout << endl;
	...
	double* z = new double[200 - count9];
	...
	for (double x = a; x <= b; x += 1)
		{
		//cout << "f(" << x << ") = " << my_lagrange(a, b, x, x_arr, z) << endl;
		fout0 << x << " " << my_lagrange(a, b, x, x_arr, z) << endl;
		}
	...
	cout << endl;
	cout << endl;
	cout << "Task number 3 (Newton1)"	<< endl;
	cout << "+ GHART!!!"	<< endl;
	cout << endl;
	...
	ofstream fout1;
	fout1.open("Newton1.txt");
	ofstream fout10;
	fout10.open("Newton10.txt");
	int a1 = 1, b1 = 6;
	for (double x = a1; x <= b1; x += 0.1)
		{
		//cout << "f(" << x << ") = " << New1(z, x) << endl;
		fout1 << x << " " << New1(z, x) << endl;
		}
	for (double x = a1; x <= b1; x += 1)
		{
		//cout << "f(" << x << ") = " << New1(z, x) << endl;
		fout10 << x << " " << New1(z, x) << endl;
		}

	ofstream fout2;
	fout2.open("Newton2.txt");
	ofstream fout20;
	fout20.open("Newton20.txt");

	cout << endl;
	cout << endl;
	cout << "Task number 4 (Newton2)"	<< endl;
	cout << "+ GHART!!!"	<< endl;
	cout << endl;
	...
	for (double x = b2; x >= a2; x -= 0.1)
		{
		//cout << "f(" << x << ") = " << New2(z, x) << endl;
		fout2 << x << " " << New2(z, x) << endl;
		}

	for (double x = b2; x >= a2; x -= 1)
		{
		//cout << "f(" << x << ") = " << New2(z, x) << endl;
		fout20 << x << " " << New2(z, x) << endl;
		}

	//5
	cout << endl;
	cout << endl;
	cout << "Task number 5 (Approximation)"	;
	cout << ", GHART!!!"	<< endl;
	cout << endl;

	ofstream file2;
	file2.open("Approximation.txt");
	ofstream file20;
	file20.open("Approximation0.txt");
	int s,month=c;
	...
	vector<int> stroki;
	int nstrok=0;
	for (int i=0; i<200; i++) if (temp[i][month]!= 999.9)
		{
	...
	for (int i=0; i<s+1; i++)
		...
		for (int j=0; j<s+1; j++)
			...
			if (i==0 && j==0) firstmat[i][j]=n1;
				...
				{
				for(int k=0; k<n1; k++) firstmat[i][j]+=pow(stroki[k], i+j);
				}
			}
		}
	...
	//cout<<"Marrich koef: "<<endl;
	//print(koef, s+1, 1);
	//cout<<endl;
	int k =stroki[n1-1];
	for (double i=0; i<k; i+=0.1) file2<<i<<"\t  "<<Steppolinom(i,s, koef)<<endl;
	file2.close();
	for (double i=0; i<k; i+=1) file20<<i<<"\t  "<<Steppolinom(i,s, koef)<<endl;
	file20.close();
